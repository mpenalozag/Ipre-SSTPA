...
R = dict()
for i in I:
  #ACA CONSEGUIMOS QUE CADA EQUIPO TENGA SU DICCIONARIO
  R[i] = dict()
  for n in N:
    R[i][n] = dict()
    #SI EL EQUIPO ES LOCAL OCURRE ESTO
    if i == matches[n]['home']:
      if matches[n]['winner'] == "H":
        ...
        R[i][n][1] = 0
        R[i][n][0] = 1
    #SI EL EQUIPO ES VISITA OCURRE ESTO
...
#1 Si el equipo puede alcanzar los t puntos en la fecha f
#0 eoc
B = {i: {t: {f: 1 if t<=team_points[i] + 3*(f-FECHAINI+1) and t>=team_points[i] else 0 for f in F} for t in T} for i in I}
